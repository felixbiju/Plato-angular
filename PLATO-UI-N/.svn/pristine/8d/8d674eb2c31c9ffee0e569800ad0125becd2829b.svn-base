...
@Component({
  selector: 'app-live-build',
  templateUrl: './live-build.component.html',
  styleUrls: ['./live-build.component.css']
})
export class LiveBuildComponent implements OnInit,OnDestroy {
  ...
  showProgress(toolStatus,toolName){
    if(toolStatus == "" && toolName != "e2e_compatibility_testing"){
      return true;
    }else{
      return false;
    }
